...
/**
 * @param {char*} S1 is a string/char array
 * @param {char*} S2 string
 * @return {void}
 * 
 * The following function takes two strings
 * and concat them together by going to the null terminator
 * character and the adds the second string and close all
 * together by finishing with the \0 character
 */
void concatenaStringhe(char* S1, char* S2) {
    while (*S1 != '\0') S1++;
